  ...
  var target = $(e.target).attr("href") // activated tab
   ...
   $('html').toggleClass('bg-yoga','bg-workout')
    ...
    function createWorkoutTimeList() {
        ...
        var time_1 = getNextWeekday(7).set('h',1).set('m',2).set('s',0).set('ms',0);
        ...
        var time_7 = getNextWeekday(2).set('h',13).set('m',32).set('s',0).set('ms',0);
        ...
        var time_5 = getNextWeekday(5).set('h',14).set('m',32).set('s',0).set('ms',0);
        time_list.push(time_5); //this is Friday 7:02 am
...
function createYogaTimeList() {
    ...
    function renderWebsite() {
        ...
        var time_list = createWorkoutTimeList();
          ...
          time_test = {
          time_Mon: getNextWeekday(7).set('h',1).set('m',2).set('s',0).set('ms',0),
          time_Mon2: getNextWeekday(2).set('h',2).set('m',32).set('s',0).set('ms',0),
          time_Tue7am: getNextWeekday(2).set('h',13).set('m',32).set('s',0).set('ms',0),
        ...
        $.each(time_list, function (idx,time) {
            ...
            if (_.isEqual(time_test["time_Mon2"], time_list[0]) == true)  {
            ...
            if (_.isEqual(time_test["time_Mon"], time_list[0]) == true)  {
            //var share_link = window.location.hostname+'/workout.html?workout=workout'+day_workout+'.json'+'&timestamp='+time.toISOString();
            var share_link = window.location.hostname+'/workout_group.html?workout=workout'+'_mon1'+'.json'+'&timestamp='+time.toISOString();
            //$("#next-"+idx).attr('href','workout.html?timestamp='+time.toISOString()+"&workout=workout"+day_workout+".json");
            $("#next-"+idx).attr('href','workout_group.html?timestamp='+time.toISOString()+"&workout=workout"+'_mon1'+".json");
            $('#next-'+idx+'-link').attr('value',share_link);}

            else if (_.isEqual(time_test["time_Tue7am"], time_list[0]) == true)  {
            //var share_link = window.location.hostname+'/workout.html?workout=workout'+day_workout+'.json'+'&timestamp='+time.toISOString();
